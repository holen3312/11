...
{
    private const TABLE_NAME = 'irk.users_activation_codes';
    ...
    public static function createActivationCode(User $user): string
    {
        ...
        $db = Db::getInstance();
        $db->query(
            'INSERT INTO ' . self::TABLE_NAME . ' (user_id, code) VALUES (:user_id, :code)', // : - в MySQL переменная,
            [
                'user_id' => $user->getId(), // разписываем шо и как
                'code' => $code
            ]
        );
